...
const featureColors = ['#1f6feb','#a78bfa','#3fb950','#d29922','#388bfd','#f78166','#a78bfa','#3fb950']
    ...
    <section id="features" className="py-24 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16 reveal">
          ...
          <p className="section-subtitle mx-auto">{f.subtitle}</p>
        </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {(f.items || []).map((item, i) => (
            <div
              key={item.title}
              className="glass-card p-6 reveal group cursor-default"
              style={{ transitionDelay: `${i * 50}ms` }}
            >
              <div
                className="w-12 h-12 rounded-xl flex items-center justify-center mb-4 transition-transform duration-300 group-hover:scale-110"
                style={{ background: `${featureColors[i]}20`, color: featureColors[i], border: `1px solid ${featureColors[i]}30` }}
              >
                {featureIcons[i]}
              </div>
              <h3 className="text-[#e6edf3] font-semibold text-base mb-2">{item.title}</h3>
